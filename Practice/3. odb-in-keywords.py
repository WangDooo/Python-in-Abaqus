...
s.Line(point1=(25.0, 0.0), point2=(Re_p1, 0.0))
...
s.Line(point1=(Re_p1, 0.0), point2=(-Re_p1, 0.0))
s.HorizontalConstraint(entity=g[11], addUndoState=False)
s.ParallelConstraint(entity1=g[10], entity2=g[11], addUndoState=False)
# session.viewports['Viewport: 1'].view.setValues(nearPlane=80.193, farPlane=108.369, width=143.172, height=67.1388, cameraPosition=(2.39696, 19.8524, 94.2809), cameraTarget=(2.39696, 19.8524, 0))
# 1.2 Generate Part-Soil cut it
p = mdb.models['Model-1'].Part(name='Part-Soil', dimensionality=TWO_D_PLANAR, type=DEFORMABLE_BODY)
...
t = p.MakeSketchTransform(sketchPlane=f[0], sketchPlaneSide=SIDE1, origin=(0.0, 25, 0.0))
s1 = mdb.models['Model-1'].ConstrainedSketch(name='__profile__', sheetSize=141.42, gridSpacing=3.53, transform=t)
g, v, d, c = s1.geometry, s1.vertices, s1.dimensions, s1.constraints
s1.setPrimaryObject(option=SUPERIMPOSE)
p = mdb.models['Model-1'].parts['Part-Soil']
p.projectReferencesOntoSketch(sketch=s1, filter=COPLANAR_EDGES)
s1.Line(point1=(-Re_p1, Re_p3), point2=(-Re_p1, -25))
...
del mdb.models['Model-1'].sketches['__profile__']
# 1.3 Draw Sketch Part-Pile
s = mdb.models['Model-1'].ConstrainedSketch(name='__profile__', sheetSize=100.0)
g, v, d, c = s.geometry, s.vertices, s.dimensions, s.constraints
s.setPrimaryObject(option=STANDALONE)
s.rectangle(point1=(0.0, 0.0), point2=(Re_pw, Re_pl))
p = mdb.models['Model-1'].Part(name='Part-Pile', dimensionality=TWO_D_PLANAR, type=DEFORMABLE_BODY)
...
mdb.models['Model-1'].materials['Material-Soil'].Elastic(table=((Re_sE, 0.4), ))
mdb.models['Model-1'].Material(name='Material-Pile')
mdb.models['Model-1'].materials['Material-Pile'].Density(table=((2500.0, ), ))
mdb.models['Model-1'].materials['Material-Pile'].Elastic(table=((34500000000.0, 0.2), ))
# 2.2 Section
...
session.viewports['Viewport: 1'].view.setValues(nearPlane=131.284, farPlane=151.559, width=115.617, height=52.0851, viewOffsetX=20.1429, viewOffsetY=22.6829)
a = mdb.models['Model-1'].rootAssembly
a.translate(instanceList=('Part-Pile-1', ), vector=(-Re_p1, Re_p2, 0.0))
# session.viewports['Viewport: 1'].view.setValues(nearPlane=125.363, farPlane=157.48, width=180.762, height=81.4325, viewOffsetX=28.4706, viewOffsetY=37.0013)
# session.viewports['Viewport: 1'].view.setValues(nearPlane=124.79, farPlane=158.053, width=179.936, height=81.0606, viewOffsetX=25.6143, viewOffsetY=4.95319)
# session.viewports['Viewport: 1'].view.setValues(nearPlane=130.216, farPlane=152.627, width=114.452, height=51.5604, viewOffsetX=16.4121, viewOffsetY=2.47917)
# session.viewports['Viewport: 1'].assemblyDisplay.setValues(adaptiveMeshConstraints=ON)

# 4. Step
mdb.models['Model-1'].StaticStep(name='Step-1', previous='Initial', maxNumInc=100000)
...
# session.viewports['Viewport: 1'].assemblyDisplay.displayGroup.replace(leaf=leaf)
a = mdb.models['Model-1'].rootAssembly
...
# session.viewports['Viewport: 1'].view.setValues(nearPlane=135.844, farPlane=146.999, width=56.8245, height=25.5992, viewOffsetX=8.17634, viewOffsetY=11.4975)
a = mdb.models['Model-1'].rootAssembly
...
# session.viewports['Viewport: 1'].view.setValues(nearPlane=136.379, farPlane=146.464, width=51.36, height=23.1375, viewOffsetX=10.5937, viewOffsetY=-16.924)
a = mdb.models['Model-1'].rootAssembly
e1 = a.instances['Part-Soil-1'].edges
edges1 = e1.getSequenceFromMask(mask=('[#602 ]', ), )
a.Set(edges=edges1, name='Set-Soil-Bottom')
# session.viewports['Viewport: 1'].view.setValues(nearPlane=127.581, farPlane=155.262, width=158.244, height=71.2883, viewOffsetX=51.155, viewOffsetY=5.70192)
a = mdb.models['Model-1'].rootAssembly
e1 = a.instances['Part-Soil-1'].edges
...
mdb.models['Model-1'].Tie(name='Constraint-P-S', master=region1, slave=region2, positionToleranceMethod=COMPUTED, adjust=ON, tieRotations=ON, thickness=ON)
# 5.3 Interaction Property
...
mdb.models['Model-1'].SurfaceToSurfaceContactStd(name='Int-P-S', createStepName='Step-1', master=region1, slave=region2, sliding=FINITE, thickness=ON, interactionProperty='IntProp-P-S', adjustMethod=NONE, initialClearance=OMIT, datumAxis=None, clearanceRegion=None)
...
mdb.models['Model-1'].DisplacementBC(name='BC-side', createStepName='Step-1', region=region, u1=0.0, u2=UNSET, ur3=UNSET, amplitude=UNSET, fixed=OFF, distributionType=UNIFORM, fieldName='', localCsys=None)
a = mdb.models['Model-1'].rootAssembly
region = a.sets['Set-Soil-Bottom']
mdb.models['Model-1'].DisplacementBC(name='BC-bottom', createStepName='Step-1', region=region, u1=0.0, u2=0.0, ur3=UNSET, amplitude=UNSET, fixed=OFF, distributionType=UNIFORM, fieldName='', localCsys=None)
...
leaf = dgm.Leaf(leafType=DEFAULT_MODEL)
# session.viewports['Viewport: 1'].assemblyDisplay.displayGroup.replace(leaf=leaf)
# session.viewports['Viewport: 1'].view.setValues(nearPlane=137.388, farPlane=145.454, width=41.0611, height=18.4979, viewOffsetX=12.996, viewOffsetY=18.4343)
# 6.2 Load
a = mdb.models['Model-1'].rootAssembly
f1 = a.instances['Part-Soil-1'].faces
faces1 = f1.getSequenceFromMask(mask=('[#7 ]', ), )
leaf = dgm.LeafFromGeometry(faceSeq=faces1)
# session.viewports['Viewport: 1'].assemblyDisplay.displayGroup.remove(leaf=leaf)
# session.viewports['Viewport: 1'].view.setValues(nearPlane=140.07, farPlane=142.773, width=13.7444, height=6.19182, viewOffsetX=4.42574, viewOffsetY=22.7823)
a = mdb.models['Model-1'].rootAssembly
...
mdb.models['Model-1'].Pressure(name='Load-pile', createStepName='Step-1', region=region, distributionType=UNIFORM, field='', magnitude=-1000.0, amplitude=UNSET)
mdb.models['Model-1'].loads['Load-pile'].setValues(magnitude=10000.0)

# 7 Mesh
# 7.1.a Seed Part - Part—Soil
...
# 7.2.a Assign Element Type -Part—Soil
...
# 7.3.a Mesh Part -Part—Soil
p = mdb.models['Model-1'].parts['Part-Soil']
p.generateMesh()
# 7.1.b Seed Part - Part—Pile
p = mdb.models['Model-1'].parts['Part-Pile']
session.viewports['Viewport: 1'].setValues(displayedObject=p)
p = mdb.models['Model-1'].parts['Part-Pile']
p.seedPart(size=0.5, deviationFactor=0.1, minSizeFactor=0.1)
# 7.2.b Assign Element Type -Part—Pile
elemType1 = mesh.ElemType(elemCode=CPE4, elemLibrary=STANDARD)
elemType2 = mesh.ElemType(elemCode=CPE3, elemLibrary=STANDARD)
p = mdb.models['Model-1'].parts['Part-Pile']
f = p.faces
...
mdb.Job(name='Re_fileName', model='Model-1', description='', type=ANALYSIS, 
	atTime=None, waitMinutes=0, waitHours=0, queue=None, memory=90, memoryUnits=PERCENTAGE, 
	...
	scratch='', resultsFormat=ODB, multiprocessingMode=DEFAULT, numCpus=20, numDomains=20, numGPUs=0)
mdb.jobs['Re_fileName'].submit(consistencyChecking=OFF)

# 9. Postprocess

odb = openOdb(path='Re_fileName.odb')
...
for uValue in u_point.values:
    uFile.write('%s,%s,%s, %f \n' % (Re_pl, Re_pw, Re_sE, uValue.data[1]))
