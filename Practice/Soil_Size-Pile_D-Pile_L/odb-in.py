...
p = mdb.models['Model-1'].parts['Part-soil']
f, e = p.faces, p.edges
t = p.MakeSketchTransform(sketchPlane=f[4], sketchUpEdge=e[7], sketchPlaneSide=SIDE1, sketchOrientation=RIGHT, origin=(1.0, 1.0, 60.0))
...
p.CutExtrude(sketchPlane=f1[4], sketchUpEdge=e1[7], sketchPlaneSide=SIDE1, sketchOrientation=RIGHT, sketch=s, depth=10.0, flipExtrudeDirection=OFF)
s.unsetPrimaryObject()
del mdb.models['Model-1'].sketches['__profile__']
p = mdb.models['Model-1'].parts['Part-soil']
c = p.cells
...
# 4. Step
mdb.models['Model-1'].StaticStep(name='Step-dym', previous='Initial', timePeriod=5.0, maxNumInc=1000, timeIncrementationMethod=FIXED, initialInc=0.02, noStop=OFF)  
