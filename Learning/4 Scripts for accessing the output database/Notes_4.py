...
4.5.2 读取场输出数据
按照下列顺序创建变量：
(1) 创建变量表示odb对象，即打开输出数据库			odb
(2) 创建变量表示第一个分析步						step 
(3) 创建变量表示第一个分析步的最后一帧				frame 
(4) 创建变量表示节点集								nodeSet
(5) 创建变量表示访问第一个分析步最后一帧的位移U		U
odb_Read_Punch.py

4.5.3 创建输出数据库并添加数据 【没啥用，就先不写了】

4.5.4 查找Mises应力的最大值
软件操作，只能显示某个分析步某一帧的最大值及其位置
脚本分析，得到所有分析步所有帧中Mises应力的最大值和出现位置

实现迭代过程顺序 1.每个分析步	2.每个分析步的每个帧	3.每个帧的Mises应力
odb_MaxMises.py

4.5.5 计算位移增量
提取位移场变量U, 对他们进行求差运算，并为该场变量创建新分析步，添加至输出数据库
U_Operation.py
